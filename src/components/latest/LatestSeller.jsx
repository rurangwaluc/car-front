  ...
  return (
            ...
            <div className={styles.box} key={user._id}>
               <Link to={`seller/${user._id}`}><img src={user.image} alt={user.name} /></Link>
                ...
                {existUser && sellerExists ? (<span key={user._id} onClick={() => handlerUnfollow(user)} className='btn-red'>Unfollow</span>) : (<span key={user._id} onClick={handlerFollow} className='btn'>Follow</span>)}
